# Álvaro Gallego Martínez - Scientific Portfolio

[![Jekyll](https://img.shields.io/badge/Jekyll-4.0+-blue.svg)](https://jekyllrb.com/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

This repository hosts my personal academic portfolio and website, showcasing my research as a bioinformatician and geneticist at the Centro Andaluz de Biología Molecular y Medicina Regenerativa (CABIMER-CSIC) in Sevilla, Spain.

## About

I am a bioinformatician and geneticist specialized in the genetic architecture of rare diseases and cancer evolution. My research focuses on identifying genetic and epigenetic determinants underlying disease mechanisms, with particular emphasis on:

- **Rare Disease Genomics**: Investigating the genetic basis of Meniere's disease, tinnitus, and vestibular disorders
- **Cancer Epigenetics**: Studying epigenetic stress resistance mechanisms in cancer (MOMENTUM Programme)
- **Computational Genetics**: Developing and applying bioinformatics approaches for variant analysis and disease gene discovery

## Research Highlights

My work spans multiple areas of human genetics and genomics:

- **Meniere's Disease**: Identified rare variants in genes such as _OTOG_, _GJD3_, and _TECTA_ associated with familial and sporadic forms
- **Tinnitus Genetics**: Characterized coding and non-coding rare variants in synaptic and neuronal genes (_ANK2_) contributing to severe tinnitus
- **Clinical Trials**: Statistical analysis and coordination for the UNITI-RCT international trial on tinnitus treatments

## Publications

My research has been published in high-impact scientific journals including _Nature Communications_, _Genome Medicine_, _npJ Genomic Medicine_, _Human Genetics_, and _EBioMedicine_. For a complete list of publications, please visit the [Publications section](https://algalliard.github.io/publications/) of my website.

## Current Position

**MOMENTUM Programme Fellow**  
Centro Andaluz de Biología Molecular y Medicina Regenerativa (CABIMER-CSIC)  
Project: _Identification of genetic and epigenetic determinants in epigenetic stress resistance in cancer_  
Supervisors: Dr. José Carlos Reyes (CSIC-CABIMER) & Dr. Juan Antonio Nepomuceno (Universidad de Sevilla)

The MOMENTUM Programme is funded by [Red.Es](https://www.red.es/) and the [PRTR](https://momentum.csic.es/programa-momentum/) through NextGeneration EU funds.

## Website Features

This portfolio website includes:

- **About**: Professional background and research interests
- **Publications**: Complete bibliography with metrics and open-access links
- **Projects**: Current and past research projects
- **Blog**: Scientific discussions and insights (in Spanish)
- **CV**: Downloadable curriculum vitae

## Building and Running

This website is built using the [al-folio](https://github.com/alshedivat/al-folio) Jekyll theme. To run locally:

```bash
# Clone the repository
git clone https://github.com/algalliard/algalliard.github.io.git

# Install dependencies
bundle install

# Serve locally
bundle exec jekyll serve
```

The site will be available at `http://localhost:4000`.

## Contact

For research inquiries, collaborations, or CV requests, please visit the [contact section](https://docs.google.com/forms/d/e/1FAIpQLSfk5lQ7sEvfgDHQMGi5MOreRsYtQVV9Bqxpug7yV2FQuLNI8Q/viewform?usp=sharing) on my website.

## License

The theme is available as open source under the terms of the [MIT License](https://github.com/alshedivat/al-folio/blob/main/LICENSE).

Originally, **al-folio** was based on the [\*folio theme](https://github.com/bogoli/-folio) (published by [Lia Bogoev](https://liabogoev.com) and under the MIT license). Since then, it got a full re-write of the styles and many additional cool features.

<p xmlns:cc="http://creativecommons.org/ns#" >This work is licensed under <a href="https://creativecommons.org/licenses/by-nc-sa/4.0/?ref=chooser-v1" target="_blank" rel="license noopener noreferrer" style="display:inline-block;">CC BY-NC-SA 4.0<img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/cc.svg?ref=chooser-v1" alt=""><img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/by.svg?ref=chooser-v1" alt=""><img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/nc.svg?ref=chooser-v1" alt=""><img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/sa.svg?ref=chooser-v1" alt=""></a></p>
